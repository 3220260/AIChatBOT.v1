...
function scorePriceFaqMatches({ normalizedMessage, searchableText, faq, faqMetaSearchText }) {
  ...
  const hasPriceIntent = /(poso|kostiz|kostos|timi|pagio)/.test(normalizedMessage);
  const hasMobileOfferContext = /(vodafone|cu|nova|q|kinito|kiniti|kartokinito|arithmo|arithmos|new number|neos arithmos|neos arithmo)/.test(normalizedMessage);
  const hasPortabilityIntent = /(foritotita|portability|metafora arithmou|metatora arithmou|fora arithmou|retain my number|kratao ton arithmo mou)/.test(normalizedMessage);
  ...
  if (hasPortabilityIntent && faq.id === "vodafone-portability-documents") {
    score += 25;
  }

  if (hasPortabilityIntent && faq.id === "nova-portability-documents") {
    score += 25;
  }
